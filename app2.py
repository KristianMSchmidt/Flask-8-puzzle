...
def index():
    data = {}
    send_new_puzzle = (request.method == 'GET'
        or (request.method == 'POST' and request.form['requested_action']=='new_puzzle'))
   
    if send_new_puzzle:
        # First visit or user has requsted a new puzzle 
        puzzle_num = random_choice(range(len(puzzle_collection)))
        puzzle = Puzzle(3,3,puzzle_collection[puzzle_num]) 
        data["puzzle_num"] = puzzle_num
        ...
        puzzle_num = request.form['puzzle_num']
        ...
        if request.form['requested_action'] == "human_move":
            ...
            for direction in ai_solution_string:
                ...
                solution_grids.append(ai_puzzle.clone()._grid)    
